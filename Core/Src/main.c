...
/**
  ...
  * Для решения поставленной задачи выбрана макетная плата NUCLEO-H743ZI2 (микроконтроллер STM32H743ZI).
	* Используется следующая перефирия TIM4, ADC1, DMA, DAC. Тактовая частота ядра была настроена 200МГц, 
	* тактовая частота шин TIM4, ADC1 100 МГц. Измерение ADC1 запускается по тригеру с таймера, который
	* устанавливается с частотой 10 кГц. Измерения ADC записываются в буфер через DMA. Буфер имеет двойной
	* размер, чтобы не останавливать измерение ADC пока идет вычисление значения. Прерывание DMA срабатывает
	* при заполнении половины буфера и полного буфера. DMA настроен на циклический режим, чтобы измерение
	* производилось без остановки. Вычиление среднего значения напряжения и вывод в терминал реализовано
